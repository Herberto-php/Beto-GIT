...
raiz.geometry(centrar_ventana(300,400,raiz))
raiz.resizable(0,0)
...
label_result.config(width=20, height=2,anchor="e")
label_result.place(x=90, y=80)

 # metodos para las operaciones
def Suma(valor1, valor2):
        #obtenemos dos valores , los sumamos y mostramos el resultado en el label_result
        global resultado
        resultado = valor1 + valor2
        label_result.config(text=resultado)

def Resta(valor1, valor2):
        #obtenemos dos valores , los restamos y mostramos el resultado en el label_result
        global resultado
        resultado = valor1 - valor2
        label_result.config(text=resultado)

def Multiplicacion(valor1, valor2):
        #obtenemos dos valores , los multiplicamos y mostramos el resultado en el label_result
        global resultado
        resultado = valor1 * valor2
        label_result.config(text=resultado)

def Division(valor1, valor2):
        #obtenemos dos valores , los dividimos y mostramos el resultado en el label_result
        global resultado
        resultado = valor1 / valor2
        label_result.config(text=resultado)

def raiz_cuadrada(valor2):
        #obtenemos un valor , le hallamos la raiz cuadrada y mostramos el resultado en el label_result
        global resultado
        resultado = math.sqrt(valor2)
        label_result.config(text=resultado)

def Potencia(valor1, valor2):
        #obtenemos un valor y lo elevamos a la potencia del segundo valor y mostramos el resultado en el label_result
        global resultado
        resultado = valor1 ** valor2
        label_result.config(text=resultado)
       ...
       #Evento de los botones
        ...
        """
        #todos estos metodos reciben el event del teclado numerico
def Codigo_boton0(event):
        global tex_valor1,tex_valor2,tex,operacion
        if operacion == "":#si no se ha presionado el boton para realizar alguna operacion el valor se agraga a la variable valor1 
                tex_valor1 += "0"
                tex = tex_valor1 + operacion + tex_valor2
                label.config(text=tex)
        else:#si se ha presionado el boton para realizar alguna operacion el valor se agraga a la variable valor2
                tex_valor2 += "0"
                tex = tex_valor1 + operacion + tex_valor2
                label.config(text=tex)
        Codigo_boton_igual("")

#la logica de todos los demas metodos de los numeros son iguales al boton cero (para no tener que comentar cada metodo para cada boton)
...
def Codigo_boton_suma(event):
    ...
    if tex_valor1=="": #comprobamos si se ha presionado algun numero antes de la operacion y si no entonces le damos el valor de 0 a la variable valor1
            tex_valor1 = "0"
    operacion = "+" #guardamos la operacion correspondiente al boton presionado
    tex = tex_valor1 + operacion + tex_valor2
    label.config(text=tex)

#la logica de todos los demas metodos de las operaciones son iguales al boton suma (para no tener que comentar cada metodo para cada operacion)
...
def Codigo_boton_borrar(event): #borramos el ultimo valor de la variable text que es la que se muestra en la pizarra
        global label,tex,tex_valor1,tex_valor2,operacion
        if operacion == "": # si no se ha presionado el boton para alguna operacion borramos del valor1 
                tex_valor1 = tex_valor1[:-1]
                tex = tex_valor1 + operacion + tex_valor2
                label.config(text=tex)
        elif tex_valor2 == "" and operacion != "":#si el valor 2 esta vacio y ya se agrego alguna operacion borramos la operacion
                operacion = ""
                tex = tex_valor1 + operacion + tex_valor2
                label.config(text=tex)
        else:#borramos el ultimo valor del valor2
                tex_valor2 = tex_valor2[:-1]
                tex = tex_valor1 + operacion + tex_valor2
                label.config(text=tex)
        Codigo_boton_igual("")

def Codigo_boton_igual(event):#enviamos los valores 1 y 2 al metodo de la operacion que se presiono . Se comprueba la operacion presionada
     ...
     #con este metodo agregamos un pto al numero para usar valores con coma
def Codigo_boton_pto(event):
        global tex_valor1,tex_valor2,tex,operacion
        
        if operacion == "":#si no se ha presionado ninguna operacion se le agrega el pto al valor 1
               if "." in tex_valor1:
                       pass
               else:
                   tex_valor1 += "."
...
boton_borrar = Button(frame, text="<-",command=lambda:Codigo_boton_borrar(""))
...
boton_igual = Button(frame, text="=", command = lambda:Codigo_boton_igual(""))
...
raiz.bind("<Key-=>",Codigo_boton_igual)
raiz.bind("<BackSpace>",Codigo_boton_borrar)
